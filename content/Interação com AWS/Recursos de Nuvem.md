Recursos de Nuvem são unidades configuráveis, como instâncias, [[Buckets S3|buckets]], bancos, redes, funções e alarmes. Cada recurso possui custo, permissões, escopo e ciclo de vida.
...
Recursos de Nuvem deve ser entendido como uma forma de conversar com a AWS. A interação pode acontecer por console, CLI, SDK, API ou infraestrutura como código, mas sempre passa por permissão e registro.

---

## Por que existe

Recursos de Nuvem existe para explicar uma forma de criar, consultar, alterar ou automatizar recursos em nuvem. Toda interação com AWS passa por [[APIs|APIs]], permissões, auditoria e algum nível de governança.

---

## Como funciona

Recursos de Nuvem funciona enviando uma solicitação para APIs da AWS com identidade, região, parâmetros e permissões. A AWS valida a chamada, executa a operação quando autorizada e registra eventos para auditoria.
...
Compare Recursos de Nuvem com console, CLI, SDK, API e IaC. Console favorece exploração visual; CLI favorece terminal e scripts; SDK fica dentro da aplicação; API é a interface base; IaC descreve infraestrutura em arquivos.
